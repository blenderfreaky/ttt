...
#[derive(Module, Debug)]
pub struct TTTMLP4State<B: Backend> {
    pub w1: Tensor<B, 4>,
    pub b1: Tensor<B, 3>,
    pub w2: Tensor<B, 4>,
    pub b2: Tensor<B, 3>,
...
impl<B: Backend> TTTInnerModel<B> for TTTMLP4<B> {
    ...
    fn new(global_config: &Arc<TTTConfig>, config: &Arc<Self::Config>, device: &B::Device) -> Self {
        ...
        Self {
            ...
            w4_init: config.initializer.init_with(
                [global_config.num_heads, mlp_dim, mlp_dim],
                Some(len),
                Some(len),
                device,
            ),
            b4_init: config.initializer.init_with(
                [global_config.num_heads, mlp_dim],
                Some(len),
                Some(len),
                device,
            ),
            w5_init: config.initializer.init_with(
                [global_config.num_heads, mlp_dim, head_dim],
                Some(len),
                Some(len),
                device,
            ),
            b5_init: config.initializer.init_with(
