...
use crate::ttt::{
    TTTConfig,
    cubecl_kernels::backend::{FusedTttBackend, api::fused_ttt_forward},
    layer::{TTTInnerModel, TTTInputsInner},
    util::{MultiHeadLayerNorm, MultiHeadLayerNormConfig},
};

#[derive(Module, Debug)]
pub struct FusedTTTLinear<B: Backend> {
...
#[derive(Module, Debug)]
pub struct FusedTTTLinearState<B: Backend> {
...
#[derive(Config, Debug)]
pub struct FusedTTTLinearConfig {
    #[config(default = "Initializer::Normal{mean:0.0, std:0.02}")]
    pub initializer: Initializer,
}

impl Default for FusedTTTLinearConfig {
    fn default() -> Self {
        Self::new()
    }
}

impl<B: FusedTttBackend> TTTInnerModel<B> for FusedTTTLinear<B> {
    type Config = FusedTTTLinearConfig;
    type State = FusedTTTLinearState<B>;
    ...
    fn init_state(&self, batch_size: usize) -> Self::State {
        ...
        FusedTTTLinearState { weight, bias }
    }

    fn forward_one(
        &self,
        state: &mut FusedTTTLinearState<B>,
    ...
    fn forward_mini_batch(
        ...
        let ln_weight = self.layer_norm.weight.val();
        let ln_bias = self.layer_norm.bias.val();
        let epsilon = self.layer_norm.epsilon as f32;
...
impl<B: FusedTttBackend> From<crate::ttt::linear::TTTLinear<B>> for FusedTTTLinear<B> {
    fn from(linear: crate::ttt::linear::TTTLinear<B>) -> Self {
        Self {
            weight_init: linear.weight_init,
            bias_init: linear.bias_init,
            layer_norm: linear.layer_norm,
