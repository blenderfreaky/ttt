...
///
/// Fused variants (FusedLinear, FusedTile, FusedTileMulti) share `TTTLinearState`
/// with the `Linear` variant since they wrap TTTLinear internally.
#[derive(Module, Debug)]
...
impl<B: FusedTttBackend> AnyInner<B> {
    ...
    pub fn from_type(inner_type: InnerModel, config: &ModelConfig, device: &B::Device) -> Self {
        match inner_type {
            ...
            InnerModel::FusedLinear => {
                let cfg = Arc::new(TTTLinearConfig::default());
                Self::FusedLinear(TTTLinear::new(config, &cfg, device).into())
            }
    ...
    pub fn init_state(&self, batch_size: usize) -> AnyInnerState<B> {
        match self {
            ...
            Self::FusedLinear(m) => AnyInnerState::Linear(m.init_state(batch_size)),
    ...
    pub fn forward(&self, state: &mut AnyInnerState<B>, inputs: TTTInputsInner<B>) -> Tensor<B, 4> {
        match (self, state) {
            ...
            (Self::FusedLinear(m), AnyInnerState::Linear(s)) => m.forward(s, inputs),
