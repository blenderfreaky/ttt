...
case "$KIT_TYPE" in
  base)
    echo "Selected: Intel OneAPI Base Toolkit"
    KIT_NAME="base"
    ...
    ;;
  hpc)
    echo "Selected: Intel OneAPI HPC Toolkit"
    KIT_NAME="hpc"
    ...
    ;;
  *)
    echo "Error: Invalid argument '$KIT_TYPE'. Please use 'base' or 'hpc'." >&2
    echo "Usage: $0 <base|hpc>" >&2
    exit 1
    ;;
esac
...
sd '(^\s*hash\s*=\s*)".*?"' "\$1\"$SRI_SHA\"" "$TARGET_FILE"
